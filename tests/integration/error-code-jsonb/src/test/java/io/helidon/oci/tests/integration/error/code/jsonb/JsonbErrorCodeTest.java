 ...
 */
package io.helidon.oci.tests.integration.error.code.jsonb;
...
import com.oracle.helidon.oci.common.errorcode.microprofile.ErrorCodeResponseMapper;
...
/**
 * This test validates that {@link com.oracle.helidon.oci.common.errorcode.ErrorDetail}
 * can be serialized and deserialized using JSON-B.
 */
@HelidonTest
@AddBean(JsonbErrorCodeTest.TestResource.class)
public class JsonbErrorCodeTest {
    ...
    void testErrorJsonp() {
        Response response = webTarget.path("error").request().get();
        assertThat(response.getStatus(), is(Response.Status.BAD_REQUEST.getStatusCode()));
        JsonObject errorDetail = response.readEntity(JsonObject.class);
        // validate that the schema of the JSON is not changed
        assertThat(errorDetail.getString("code"), is("InvalidParameter"));
        assertThat(errorDetail.getString("message"), is("Invalid parameter sent"));
    }
    ...
    @Path("/")
    @RegisterProvider(ErrorCodeResponseMapper.class)
    // maps to RenderableException
