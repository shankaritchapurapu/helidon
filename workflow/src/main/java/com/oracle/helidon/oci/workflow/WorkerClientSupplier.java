...
/**
 * Provides a named Workflow worker client compatible with the legacy WFaaS binding name.
 */
@Service.Singleton
...
public class WorkerClientSupplier implements Supplier<WorkflowClient> {
    ...
    @Service.Inject
    WorkerClientSupplier(WorkflowClientFactory workflowClientFactory) {
        this.workflowClientFactory = workflowClientFactory;
    }

    @Override
    public WorkflowClient get() {
        return workflowClientFactory.createWorkflowClient(ClientRole.WORKER);
    }
}
