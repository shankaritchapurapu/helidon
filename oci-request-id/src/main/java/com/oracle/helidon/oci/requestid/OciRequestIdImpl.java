...
record OciRequestIdImpl(String customerId, String traceId, String spanId) implements OciRequestId {
    ...
    static OciRequestId parseUpstreamRequest(String headerValue) {
        ...
        if (parts.length == 3) {
            ...
            warnInvalid("Received upstream request id with spanId.", headerValue);
        } else {
            // this is invalid, wrong number of parts
            warnInvalid("Received upstream request id with to many elements: " + parts.length + ".", headerValue);
        }
        return new OciRequestIdImpl(clean(parts[0]), clean(parts[1]), generateUniqueId());
    }
