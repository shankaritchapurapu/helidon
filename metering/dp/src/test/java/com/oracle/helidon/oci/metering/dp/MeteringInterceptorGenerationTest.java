...
class MeteringInterceptorGenerationTest {
    ...
    @Test
    void generatedPointInterceptorRecordsWithNativeRecorder() throws Exception {
        LogFileUsageRecorder recorder = mock(LogFileUsageRecorder.class);
        when(recorder.recordMetersAsync(anyList())).thenReturn(() -> true);
        Services.set(LogFileUsageRecorder.class, recorder);
        ...
        ArgumentCaptor<List<Meters>> metersCaptor = ArgumentCaptor.captor();
        verify(recorder).recordMetersAsync(metersCaptor.capture());
        List<Meters> meters = metersCaptor.getValue();

        assertThat(result, is("ok"));
        assertThat(meters, hasSize(1));
        assertThat(meters.getFirst().getService(), is("inventory"));
        assertThat(meters.getFirst().getMetrics(), hasSize(1));
        assertThat(meters.getFirst().getMetrics().getFirst().getMtr(), is("requests"));
        assertThat(meters.getFirst().getMetrics().getFirst().getCompartmentId(), is("ocid1.compartment.oc1..example"));
        assertThat(meters.getFirst().getMetrics().getFirst().getResourceId(), is("resource-1"));
        assertThat(meters.getFirst().getMetrics().getFirst().getValue(), is(3.5F));
        assertThat(meters.getFirst().getMetrics().getFirst().getTags(), containsString("\"source\":\"test\""));
        assertThat(meters.getFirst().getMetrics().getFirst().getTags(), containsString("\"operation\":\"create\""));
    }
    ...
    void generatedStartAndEndInterceptorsUseContextRegion() throws Exception {
        ...
        assertThat(meters, hasSize(1));
        assertThat(meters.getFirst().getMetrics(), hasSize(1));
        assertThat(meters.getFirst().getMetrics().getFirst().getMtr(), is("duration"));
        assertThat(meters.getFirst().getMetrics().getFirst().getCompartmentId(), is("ocid1.compartment.oc1..example"));
        assertThat(meters.getFirst().getMetrics().getFirst().getResourceId(), is("resource-1"));
        assertThat(meters.getFirst().getMetrics().getFirst().getTags(), containsString("\"phase\":\"create\""));
        assertThat(meters.getFirst().getMetrics().getFirst().getTags(), containsString("\"end\":\"true\""));
        assertThat(meters.getFirst().getMetrics().getFirst().getTags(), containsString("\"result\":\"ok\""));
    }

    @Test
    void generatedStartInterceptorRejectsNestedContextRegion() throws Exception {
        Services.set(LogFileUsageRecorder.class, mock(LogFileUsageRecorder.class));
        Services.set(MeteringConfig.class, MeteringConfig.builder()
                .endpoint("https://bling-dp.example")
                .build());
        Interception.ElementInterceptor start = generatedInterceptor("Metering Start interceptor");
