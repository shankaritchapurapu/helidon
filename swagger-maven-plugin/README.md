...
The model classes are based entirely on the Public SDK code generator, and will look like what our public Java SDK users would
see.
...
The Swagger (OpenAPI) generation can be configured by using the `helidon-oci-swagger-maven-plugin`
with `oracle-java-helidon-service` set as the language.
...
- language (defaults to oracle-java-helidon-service)- Generator to use. Must be set to `oracle-java-helidon-service` to generate
  Helidon MP sources.
...
- basePackage (Required) - The base package for all generated sources. The api files will be written to the `${basePackage}.api`
  and models to the `${basePackage}.model` packages.
...
- Enables 'enableDefaultDiscriminatorOnlyElseNone' - this is used to prevent defaulting of polymorphic types to a specific class
  type unless the spec explicitly says there's a default (for backwards compat).
...
- 'contexts*' (default false) - these are various other flags to inject other different types of contexts, see
  OracleJavaHelidonServiceCodegen.ConfigOption enum for more info.
- 'useJaxRsServiceResponse' (default false) - whether to wrap all return types of all operations in `jakarta.ws.rs.core.Response`
  objects.

The generated service stubs currently result in an abstract classes you can extend from. It is recommend you extend from '
Abstract<yourservice>BaseResource'. This is the latest version that allows you the most ability to customize the injected
contexts.
