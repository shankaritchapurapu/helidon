# Helidon OCI Documentation: Utilities

| Topic                                     | Description                                                            |
|-------------------------------------------|------------------------------------------------------------------------|
| [Environment Config](environment-config.md) | OCI-aware Helidon config source and region providers.                  |
| [Error Code](error-code.md)               | Support for OCI-standard error code handling in Helidon services.      |
| [Request ID](request-id.md)               | `opc-request-id` propagation, generation, and logging context support. |
| [OCI SDK](oci-sdk.md) | Shared OCI SDK authentication provider integration, including service-principal support. |

