...
/**
 * Runtime config helpers for OCI metrics blueprints.
 */
final class ConfigSupport {
    ...
    static final class ClientConfigurationSupport {
        ...
        static ClientConfiguration createClientConfiguration(OciClientConfiguration config) {
            ...
            config.connectionTimeout()
                    .ifPresent(timeout -> builder.connectionTimeoutMillis((int) timeout.toMillis()));
            config.maxAsyncThreads().ifPresent(builder::maxAsyncThreads);
            config.readTimeout()
                    .ifPresent(readTimeout -> builder.readTimeoutMillis((int) readTimeout.toMillis()));
    ...
    static final class OciMetricsPublisherConfigSupport {

        private OciMetricsPublisherConfigSupport() {
        }

        @Prototype.RuntimeTypeFactoryMethod("clientConfiguration")
        static ClientConfiguration createClientConfiguration(OciClientConfiguration config) {
            return ConfigSupport.ClientConfigurationSupport.createClientConfiguration(config);
        }
